...
def test_generalized_weighted_kernel_type2():
    ...
    geometric_weight, spectral_matrix, kernel_model_s, _, kernel_model_x = _test_kernel_model(seed=0)
    alpha = 1.0
    ...
    assert jnp.sum(result) == 106.90668, f"Expected sum 106.90668, got {jnp.sum(result)}"
...
def _test_kernel_model(Ni=2, Nj=3, Nk=4, Nl=5, seed=0):
    ...
    return geometric_weight, spectral_matrix, kernel_model_s, kernel_model_t, kernel_model_x
